...
public:
	...
	/** * @brief UWeaponPoolSubsystem에서 사용할 풀 크기를 설정하는 변수 */
	UPROPERTY(EditAnywhere, Category = "Weapon")
	int32 WeaponPoolSize;

	/** * @brief 사운드와 나이아가라 애셋을 참조하는 변수 */
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category = "Weapon|Effect|Sound", meta = (AllowedTypes = "SoundDataAsset"))
	FPrimaryAssetId AttackSoundId;

	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category = "Weapon|Effect|Niagara", meta = (AllowedTypes = "NiagaraDataAsset"))
...
private:
	 ...
	 * - BuildAttackInfo에서 공격 정보를 가져온 후 SpawnWeapons로 넘겨주며 공격을 실행
	 */
	void ExecuteAttack();
	 ...
	 */
	void SpawnWeapons(const FPetAttackInfo& AttackInfo, const FVector& TargetLocation);
